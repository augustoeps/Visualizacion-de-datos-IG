...
function dibujarLinea(x1, y1, x2, y2, ) {
   //console.log("Color de la línea:", color);  // Verificar el color
...
function filtraparadasActivas() {
  ...
  for (const line of lineas) scene.remove(line);
  lineas = [];
  ...
  if (registrosFiltrados.length > 0) {
    ...
    for (const estacion of datosEstaciones) {
      if (estacionesA.has(estacion.nombre) || estacionesB.has(estacion.nombre)) {
        ...
        else if (estacionesA.has(estacion.nombre))
          objetos[i].material.color.set(0x00ff00); // color para estaciones de inicio
        else
          objetos[i].material.color.set(0xffff00); // color para estaciones de fin
      }
      i++;
    }

    // Dibuja líneas entre estaciones de inicio y fin para cada alquiler activo
    for (const registro of registrosFiltrados) {
      const estacionInicio = datosEstaciones.find(est => est.nombre === registro.p_inicio);
      const estacionFin = datosEstaciones.find(est => est.nombre === registro.p_fin);

      if (estacionInicio && estacionFin) {
        const x1 = Mapeo(estacionInicio.lon, minlon, maxlon, -mapsx / 2, mapsx / 2);
        const y1 = Mapeo(estacionInicio.lat, minlat, maxlat, -mapsy / 2, mapsy / 2);
        const x2 = Mapeo(estacionFin.lon, minlon, maxlon, -mapsx / 2, mapsx / 2);
        const y2 = Mapeo(estacionFin.lat, minlat, maxlat, -mapsy / 2, mapsy / 2);
        
        // Llama a la función para dibujar la línea
        dibujarLinea(x1, y1, x2, y2, 0xffffff);
      }
    }
  }
}
